...
`,
      ...
      // v2026.3.24+: runtime/index.ts uses VERSION from version.ts directly.
      // The version is injected via the `define` option in Bun.build() below.
      ...
      // --- Playwright CJS interop fix ---
      // Bun generates both static and lazy (__esm) copies. The static copy
      // destructures `const { chromium } = playwright` before playwright is
      // initialized. Inline all usage to `playwright.chromium.<method>`.
      build.onLoad({ filter: /browser[/\\]pw-session\.ts$/ }, (args) => {
        let src = readFileSync(args.path, "utf-8");
        src = src.replace(/^import\s+playwright\s+from\s+["']playwright-core["'];?/m, `import playwright from "playwright-core";\n// @ts-ignore — Bun CJS interop: unwrap default\nconst __pw: typeof import("playwright-core") = (playwright as any).default ?? playwright as any;`);
        src = src.replace(/const\s+\{\s*chromium\s*\}\s*=\s*playwright;\n?/g, "");
        src = src.replace(/\bchromium\b\./g, "__pw.chromium.");
        return { contents: src, loader: "ts" };
      });
      build.onLoad({ filter: /browser[/\\]pw-tools-core\.state\.ts$/ }, (args) => {
        let src = readFileSync(args.path, "utf-8");
        src = src.replace(/^import\s+playwright\s+from\s+["']playwright-core["'];?/m, `import playwright from "playwright-core";\nconst __pw: typeof import("playwright-core") = (playwright as any).default ?? playwright as any;`);
        src = src.replace(/const\s+\{\s*devices:\s*playwrightDevices\s*\}\s*=\s*playwright;\n?/g, "");
        src = src.replace(/\bplaywrightDevices\b/g, "__pw.devices");
        return { contents: src, loader: "ts" };
      });
      ...
      // NOTE: file-type/core.js is pre-patched on disk (patchFileTypeCoreOnDisk)
      // because Bun validates ESM exports before any plugin hooks fire.
      ...
      // Skip --disable-warning=ExperimentalWarning respawn under Bun
      build.onLoad({ filter: /entry\.respawn\.ts$/ }, (args) => ({
        contents: patchEntryRespawn(readFileSync(args.path, "utf-8")),
        loader: "ts",
      }));
...
// Pre-patch files on disk (Bun validates ESM exports / generates static copies before plugins fire)
const restoreFileType = patchFileTypeCoreOnDisk();
