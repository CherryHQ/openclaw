...
export function patchEntryTs(
  ...
  const lines: string[] = [
    ...
    `import __shimFsPromises from "node:fs/promises";`,
...
function buildVfsMonkeyPatches(): string[] {
  return [
    ...
    `const __patchedRealpathSync = function(p: any, options?: any): any {`,
    `  if (typeof p === "string") {`,
    `    const hit = __vfsLookup(p);`,
    ...
    `};`,
    `// Preserve .native so callers of fs.realpathSync.native() don't break`,
    `__patchedRealpathSync.native = __origRealpathSync.native;`,
    `(__shimFs as any).realpathSync = __patchedRealpathSync;`,
    ``,
    ...
    `};`,
    ``,
    `// Also patch node:fs/promises (separate module object in Bun compiled binaries)`,
    `const __origFspAccess = __shimFsPromises.access;`,
    `(__shimFsPromises as any).access = async function(p: any, mode?: any): Promise<void> {`,
    `  if (typeof p === "string") {`,
    `    const hit = __vfsLookup(p);`,
    `    if (hit && (hit.rel in hit.dirManifest || hit.rel in hit.fileMap)) return;`,
    `  }`,
    `  return __origFspAccess.call(__shimFsPromises, p, mode);`,
    `};`,
    `const __origFspReadFile = __shimFsPromises.readFile;`,
    `(__shimFsPromises as any).readFile = async function(p: any, options?: any): Promise<any> {`,
    `  if (typeof p === "string") {`,
    `    const hit = __vfsLookup(p);`,
    `    if (hit && hit.rel in hit.fileMap) {`,
    `      return __origFspReadFile.call(__shimFsPromises, hit.fileMap[hit.rel]!, options);`,
    `    }`,
    `  }`,
    `  return __origFspReadFile.call(__shimFsPromises, p, options);`,
    `};`,
    ``,
