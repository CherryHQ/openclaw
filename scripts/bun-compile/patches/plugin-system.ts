...
export function patchLoaderTs(
  ...
  const patched = source
    ...
    .replace(
      ...
      (match) => {
        ...
        return [
          ...
          `      // Always use candidate.source for bundled extensions — setupEntry is`,
          `      // not prebundled; the main index.ts handles both full and setup paths.`,
    ...
    // 3b. Clear setupSource for bundled extensions — setupEntry files are not
    //     prebundled into VFS, so the loader must NOT take the setup-only branch
    //     (which would skip pushing to registry.channels and leave the plugin
    //     without actions/capabilities in channel tool discovery).
    .replace(
      /if \(\s*\(registrationMode === "setup-only" \|\| registrationMode === "setup-runtime"\)\s*&&\s*\n?\s*manifestRecord\.setupSource\s*\)/,
  ...
  // 5. Replace resolveCreatePluginRuntime to directly import from the bundled
  //    runtime module instead of filesystem resolution + jiti (which fails in $bunfs).
  const runtimePatched = patched.replace(
    /const resolveCreatePluginRuntime = \(\)[\s\S]*?return createPluginRuntimeFactory;\n\s*\};/,
    [
      `const resolveCreatePluginRuntime = (): ((options?: CreatePluginRuntimeOptions) => PluginRuntime) => {`,
      `    if (createPluginRuntimeFactory) return createPluginRuntimeFactory;`,
      `    const { createPluginRuntime: __cpr } = require("./runtime/index.js") as { createPluginRuntime: (options?: CreatePluginRuntimeOptions) => PluginRuntime };`,
      `    createPluginRuntimeFactory = __cpr;`,
