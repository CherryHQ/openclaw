...
// file-type/core.js fix is handled by patchFileTypeCoreOnDisk() in the parent script
...
export async function bundleExtensions(
  ...
  const manifestOnlyDirs: { name: string; dir: string }[] = [];
  for (const entry of readdirSync(extensionsDir, { withFileTypes: true })) {
    ...
    if (!entryFile || !existsSync(entryFile)) {
      // Manifest-only extensions (config-only plugins with no code entry).
      // Copy just the manifest so the plugin loader can discover them in VFS.
      const outDir = join(extTempDir, entry.name);
      mkdirSync(outDir, { recursive: true });
      copyFileSync(manifestFile, join(outDir, "openclaw.plugin.json"));
      manifestOnlyDirs.push({ name: entry.name, dir: outDir });
      continue;
    }
  ...
  for (const ext of extDirs) {
    ...
    // Post-process: rewrite dynamic import("openclaw/plugin-sdk/...") to require()
    // so they go through the CJS proxy require at runtime (which redirects to the
    // extracted SDK dir). Bun's CJS format preserves `await import()` literally,
    // but the compiled binary can't resolve bare "openclaw/plugin-sdk/*" specifiers.
    const bundledEntry = join(outDir, entryJsName);
    if (existsSync(bundledEntry)) {
      let code = readFileSync(bundledEntry, "utf-8");
      const patched = code.replace(
        /\bimport\s*\(\s*["']openclaw\/plugin-sdk(\/[^"']+)?["']\s*\)/g,
        (_m, sub) => {
          const spec = sub ? `openclaw/plugin-sdk${sub}` : "openclaw/plugin-sdk";
          return `Promise.resolve(require(${JSON.stringify(spec)}))`;
        },
      );
      if (patched !== code) {
        writeFileSync(bundledEntry, patched);
      }
    }
  ...
  // Include manifest-only extensions (no code, just plugin.json)
  for (const m of manifestOnlyDirs) {
    results.push({ name: m.name, dir: m.dir, entryJs: "" });
  }
  return results;
}
